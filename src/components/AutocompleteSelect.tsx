...
export const AutocompleteSelect: React.FC<AutocompleteSelectProps> = ({
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (e.key === 'ArrowDown') {
      ...
      if (isOpen) {
        // If exact match on tab, select it
    ...
    <div ref={containerRef} className="relative w-full">
      ...
      <div className="relative">
        ...
        <input
          ...
          aria-invalid={!!error}
          className={`w-full pl-9 pr-10 py-2.5 bg-white text-slate-900 font-medium text-sm rounded-lg border shadow-xs transition-all outline-none uppercase placeholder:normal-case placeholder:text-slate-400 ${
            error
      ...
      {isOpen && !disabled && (
        <ul
          id={listboxId}
          role="listbox"
          className="absolute z-40 mt-1 w-full max-h-60 overflow-auto bg-white border border-slate-200 rounded-lg shadow-xl py-1 text-sm text-slate-800"
        >
